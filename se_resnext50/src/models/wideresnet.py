...
class WideBasic(nn.Cell):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.conv1 = nn.Conv2d(
            in_channels, out_channels, kernel_size=3, stride=stride, has_bias=True
        )
        self.bn2 = nn.BatchNorm2d(out_channels)
        self.dropout = nn.Dropout(keep_prob=0.7)
        self.conv2 = nn.Conv2d(
            out_channels, out_channels, kernel_size=3, stride=1, has_bias=True
        )
...
def wideresnet_d4w10(num_classes=2388):
    depth = 40
    widen_factor = 10
