...
class Stem(nn.Cell):
    ...
    def __init__(self, in_channels):
        ...
        self.conv2d_2b = Conv2d(
            32, 64, kernel_size=3, stride=1, padding=1, pad_mode="pad"
        )
...
class Inception_resnet_v2(nn.Cell):
    ...
    def __init__(
        self, in_channels=3, classes=1000, k=192, l=224, m=256, n=384, is_train=True
    ):
        ...
        self.conv2d_7b = Conv2d(2080, 4096, kernel_size=1, stride=1)
        self.avgpool = P.ReduceMean(keep_dims=False)
        self.softmax = nn.DenseBnAct(
            4096, classes, weight_init="XavierUniform", has_bias=True, has_bn=False
        )
        if is_train:
...
def inception_resnet_v2_wider(num_classes=2388):
    return Inception_resnet_v2(classes=num_classes)
