...
def build_optimizer(config, lr, network):
...
def build_lr_scheduler(config):
    if config.lr_scheduler == "multistep":
        ...
        print("use cosine_annealing lr scheduler")
        ...
        print("use cosine_annealing_V2 lr scheduler")
        ...
        print("use cosine_annealing_sample lr scheduler")
...
class _LinearWarmUp(_WarmUp):
    ...
    def get_lr(self, current_step=0):
        lr_inc = (float(self.base_lr) - float(self.warmup_init_lr)) / float(
            self.warmup_steps
        )
...
class StepLR(_LRScheduler):
    ...
    def __init__(
        self, lr, epoch_size, gamma, steps_per_epoch, max_epoch, warmup_epochs=0
    ):
...
class POLYLR(_LRScheduler):
    ...
    def get_lr(self):
        ...
        for i in range(total_steps):
            ...
            lr_each_step.append(
                (self.lr - self.end_lr) * ((1.0 - step_ / total_steps) ** self.power)
                + self.end_lr
            )
...
class MultiStepLR(_LRScheduler):
    ...
    def __init__(
        self, lr, milestones, gamma, steps_per_epoch, max_epoch, warmup_epochs=0
    ):
...
class CosineAnnealingLR(_LRScheduler):
    ...
    def __init__(
        self, lr, t_max, steps_per_epoch, max_epoch, warmup_epochs=0, eta_min=0
    ):
        self.t_max = t_max
            ...
            if i < warmup_steps:
                ...
                if i % self.steps_per_epoch == 0 and i > 0:
                    current_lr = (
                        ...
                        * (1.0 + math.cos(math.pi * cur_ep / self.t_max))
                        / 2
                    )
...
class CosineAnnealingWarmRestarts(_LRScheduler):
    ...
    def __init__(
        self, lr, steps_per_epoch, max_epoch, t_0, t_mult=1, eta_min=0, warmup_epochs=0
    ):
    ...
    for i in range(total_steps):
        ...
        if i < warmup_steps:
            ...
            lr = (
                eta_min
                + (base_lr - eta_min)
                * (1.0 + math.cos(math.pi * last_epoch / t_max))
                / 2
            )
        lr_each_step.append(lr)
